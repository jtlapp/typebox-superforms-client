...
import { JavaScriptType, TypeBoxType } from "./internal-types.js";
...
export function unspecifiedValue(fieldInfo: FieldInfo): unknown {
...
function createFieldInfo(fieldName: string | null, schema: TSchema): FieldInfo {
  ...
  if (isNullable) {
    if (typeBoxType === TypeBoxType.Boolean) {
      throw Error("Boolean types (checkboxes) are not nullable");
    }
    if (isOptional) {
      throw Error("Types cannot be both optional and nullable");
    }
  }
